...
const projects = [
  {
    title: "Residential Marble Floor",
    label: "Marble Floor Polish",
    before: stonecareImageAssets.beforeAfter.before1,
    after: stonecareImageAssets.beforeAfter.after1,
  },
  // {
  ...
  // },
  {
    ...
    before: stonecareImageAssets.beforeAfter.before3,
    after: stonecareImageAssets.beforeAfter.after3,
  },
];
...
function Slider({
  ...
  return (
    <div
      ref={containerRef}
      className="relative mx-auto w-full max-w-4xl cursor-col-resize select-none overflow-hidden border border-border"
    ...
    >
      ...
      <div className="absolute left-1/2 top-3 z-10 max-w-[calc(100%-1.5rem)] -translate-x-1/2 bg-white/85 px-3 py-1.5 text-center text-[10px] font-display uppercase tracking-[0.14em] text-black sm:top-4 sm:px-4 sm:py-2 sm:text-sm sm:tracking-[0.18em]">
        {title}
      </div>
    </div>
  );
          ...
          <div className="flex flex-col gap-3">
            ...
            {projects.map((p, i) => (
              <button
                key={p.label}
                onClick={() => setActive(i)}
                className={`group relative flex items-center gap-3 border p-4 text-left transition-all duration-200 sm:gap-4 ${
                  i === active
